...
function comisoma_activation()
{
	...
	if (!is_array($comisoma_content) || empty($comisoma_content)) {
		$comisoma_content = array(
			...
			'countdown_date' => gmdate('Y-m-d', strtotime('+1 month')),
...
function comisoma_menu_page()
{
	...
	add_submenu_page('comisoma', 'Our Plugins', 'Our Plugins', 'manage_options', 'comisoma-our-plugins', 'comisoma_our_plugins_page');
	add_submenu_page('comisoma', 'Our Themes', 'Our Themes', 'manage_options', 'comisoma-our-themes', 'comisoma_our_themes_page');
}
...
// Our Plugins page body
function comisoma_our_plugins_page()
{
	require 'admin/our-plugins.php';
}

// Our Themes page body
function comisoma_our_themes_page()
{
	require 'admin/our-themes.php';
}
	...
	if (current_user_can('manage_options')) {
		...
		if (isset($_GET['page'])) {
			...
			if (strpos($sf_current_page_slug, 'comisoma') !== false || $sf_current_page_slug === 'comisoma-pro') {
				...
				if ($sf_current_page_slug === 'comisoma') {
					...
					wp_enqueue_script('comisoma-sweetalert2-js', plugin_dir_url(__FILE__) . 'admin/assets/js/sweetalert2.all.min.js', array(), '11.26.25', true);
				...
				if ($sf_current_page_slug === 'comisoma-our-plugins' || $sf_current_page_slug === 'comisoma-our-themes') {
					// Custom CSS for Our Plugins and Our Themes pages
					wp_enqueue_style('comisoma-themes-plugins-style-css', plugin_dir_url(__FILE__) . 'admin/assets/css/themes-plugins-style.css', array('comisoma-bootstrap-admin-css'), '1.0.0');
				}
...
function comisoma_logo_li_callback()
{
	...
	if (isset($_POST['attachment_id'])) {
		...
		?>
		<li class="col-md-4 comisoma-logo-<?php echo esc_attr($comisoma_logo_id); ?>"
			data-position="<?php echo esc_attr($comisoma_logo_id); ?>">
			<input type="hidden" class="form-control comisoma-logo-id" id="comisoma-logo-id" name="comisoma-logo-id"
				value="<?php echo esc_attr($comisoma_logo_id); ?>">
			<img src="<?php echo esc_url($comisoma_logo_url[0]); ?>" class="img-thumbnail mt-3 bg-light">
			<div class="d-grid gap-2">
				<button type="button" id="comisoma-remove-logo"
					onclick="comisoma_save('remove-logo', <?php echo esc_attr($comisoma_logo_id); ?>);"
					class="btn btn-danger btn-block"><i class="fa-solid fa-trash"></i>
					<?php esc_html_e('Remove Logo', 'coming-soon-maintenance-mode'); ?></button>
			</div>
		</li>
		<?php
		wp_die();
	...
	if (current_user_can('manage_options')) {
		if (isset($_POST['nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'comisoma-save')) {
			...
			if ($tab == 'reset') {
				...
				update_option('comisoma_content', $default_content);
			}
			// reset data save end
			...
			if ($tab == 'import') {
				...
				if (!empty($import_data)) {
					...
					if (is_array($imported_options)) {
						if (isset($imported_options['comisoma_settings']))
							update_option('comisoma_settings', $imported_options['comisoma_settings']);
						if (isset($imported_options['comisoma_templates']))
							update_option('comisoma_templates', $imported_options['comisoma_templates']);
						if (isset($imported_options['comisoma_content']))
							update_option('comisoma_content', $imported_options['comisoma_content']);
						if (isset($imported_options['comisoma_social_media']))
							update_option('comisoma_social_media', $imported_options['comisoma_social_media']);
...
$comisoma_launch_dt = gmdate('F d, Y H:i:s', strtotime("$comisoma_countdown_date $comisoma_countdown_time"));
//load CSMM content
$comisoma_content = array();
...
if (is_array($comisoma_content)) {
	...
	if (array_key_exists('countdown', $comisoma_content)) {
		$comisoma_countdown = $comisoma_content['countdown'];
	}
	if (array_key_exists('countdown_date', $comisoma_content) && !empty($comisoma_content['countdown_date'])) {
		$comisoma_countdown_date = $comisoma_content['countdown_date'];
	}
	if (array_key_exists('countdown_time', $comisoma_content) && !empty($comisoma_content['countdown_time'])) {
		$comisoma_countdown_time = $comisoma_content['countdown_time'];
	}
...
if (is_array($comisoma_settings)) {
	if (array_key_exists('website_mode', $comisoma_settings)) {
		$comisoma_website_mode = $comisoma_settings['website_mode'];
	}
}
	...
	function comisoma_website_mode()
	{
		...
		if (!is_user_logged_in()) {
			...
			if (is_array($comisoma_settings)) {
				...
				if (array_key_exists('selected_posts', $comisoma_settings)) {
					$comisoma_posts = $comisoma_settings['selected_posts'];
				}
				if (array_key_exists('selected_pages', $comisoma_settings)) {
					$comisoma_pages = $comisoma_settings['selected_pages'];
				}
				if (array_key_exists('selected_other_pages', $comisoma_settings)) {
					$comisoma_other_pages = $comisoma_settings['selected_other_pages'];
				}
			}
